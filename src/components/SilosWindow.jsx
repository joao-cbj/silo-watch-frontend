...
import SiloListTab from "./silos/tabs/SiloListTab";
import AddSiloTab from "./silos/tabs/AddSiloTab";
import IntegrationTab from "./silos/tabs/IntegrationTab";

const SilosWindow = ({ onClose }) => {
  const [activeTab, setActiveTab] = useState(null);
  const [refreshTrigger, setRefreshTrigger] = useState(0);

  const handleSiloCreated = () => {
    setRefreshTrigger(prev => prev + 1);
    setActiveTab("silolist");
  };
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      case "silolist":
        return <SiloListTab key={refreshTrigger} />;
      case "add":
        return <AddSiloTab onSiloCreated={handleSiloCreated} />;
      case "integration":
  ...
  const menuItems = [
    {
      id: "silolist",
      label: "Lista de Silos",
      icon: ListBulletIcon,
      enabled: true,
    },
    {
      id: "add",
      label: "Adicionar Silo",
      icon: PlusCircleIcon,
      enabled: true,
    },
    {
      ...
      >
        ...
        >
          ...
          <div className="flex items-center px-5 py-4 border-b border-gray-400/30">
            <ArchiveBoxIcon className="h-5 w-5 mr-2 text-blue-100" />
            <span className="font-semibold text-blue-50">Silos</span>
          </div>
